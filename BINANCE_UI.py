...
with open(SECRETS_PATH) as f:
    vals = json.load(f)
    api_key = vals['BINANCE_API_KEY']
    api_secret = vals['BINANCE_API_SECRET']
client = BC(api_key, api_secret)
...
def sell_all():
    ...
    for asset,row in bal.iterrows():
        if asset == vals["Base Currency"]:
            continue
        free = row["free"]
        sym  = asset + vals["Base Currency"]
        try:
...
with col1:
    symbol = st.text_input("Symbol (e.g. BTCUSDT)", value=f"BTC{vals['Base Currency']}").upper()
    usdt_amt = st.number_input(f"Amount in {vals['Base Currency']}", min_value=0.0, step=1.0, format="%.2f")
